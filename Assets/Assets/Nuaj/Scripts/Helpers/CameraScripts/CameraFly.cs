...
/// It shows how to hook to the NuajManager and NuajOrchestrator for external drive and also manages the lightning bolt strikes intervals.
/// 
...
public class CameraFly : MonoBehaviour
{
	...
	void	Update()
	{
		...
		bool	bIsButtonPressed = ButtonDown || Input.GetMouseButton( 1 ) || Input.GetMouseButton( 2 );
		if ( m_bDemoMode && bIsButtonPressed )
		{	// Exit demo mode
			m_bDemoMode = false;
			m_TimeFromLastClick = Now;
			Start();
		}
		if ( !bIsButtonPressed && (Now - m_TimeFromLastClick).TotalSeconds > 20 )
...
//		// Key dropper that populates an array of keys on middle button click for the demo...
...
//protected List<Vector3>		m_RotationKeys = new List<Vector3>();
	...
	protected class		DemoPath
	{
		...
		// 3D Noise tiling factors
		public float 			NoiseTilingFactorStart = 1.0f, NoiseTilingFactorEnd = 1.0f;


		// Interpolation cached data
		protected bool			m_bPathPreProcessed = false;
		protected Vector3[]		m_PositionTangents = null;
		protected Quaternion[][]	m_RotationTangents = null;
		...
		public void		Apply( NuajManager _Manager, NuajOrchestrator _Orch, float _Time )
		{
			...
			float	PathInterpolant = _Time / Duration;

			_Orch.WeatherTypeSource = PresetSource;
			_Orch.WeatherTypeTarget = PresetTarget;
			_Orch.WeatherBalance = Mathf.Lerp( PresetBalanceStart, PresetBalanceEnd, PathInterpolant );

			_Orch.TimeOfDay = Mathf.Lerp( TimeOfDayStart, TimeOfDayEnd, PathInterpolant );
			_Orch.DensityMultiplierAtNight = NightDensityFactor;

			_Orch.WindForce = Wind;
			_Orch.WindDirectionAngle = WindAngle;

			// Tweak 3D noise tiling
			_Orch.NoiseTilingFactor = Mathf.Exp( Mathf.Lerp( Mathf.Log( NoiseTilingFactorStart), Mathf.Log( NoiseTilingFactorEnd ), PathInterpolant ) );	// We're interpolating a factor here !
			...
			Vector3	Position = CSplinePosition( ref PositionKeys[m_KeyIndex], ref PositionKeys[m_KeyIndex+1], ref m_PositionTangents[m_KeyIndex], ref m_PositionTangents[m_KeyIndex+1], t );
			Vector3	View = QuatSquad( ref m_RotationTangents[m_KeyIndex][0], ref m_RotationTangents[m_KeyIndex][3], ref m_RotationTangents[m_KeyIndex][1], ref m_RotationTangents[m_KeyIndex][2], t );
		...
		protected void	PreProcessPath()
		{
			...
			// Build position tangents
			m_PositionTangents = new Vector3[PositionKeys.Length];
			if ( PositionKeys.Length > 1 )
			{
				for ( int KeyIndex=0; KeyIndex < PositionKeys.Length; KeyIndex++ )
				{
					if ( KeyIndex == 0 )
					{	// First tangent
						m_PositionTangents[KeyIndex] = PositionKeys[KeyIndex+1] - PositionKeys[KeyIndex];
					}
					else if ( KeyIndex == PositionKeys.Length-1 )
					{	// Last tangent
						m_PositionTangents[KeyIndex] = PositionKeys[KeyIndex] - PositionKeys[KeyIndex-1];
					}
					else
					{	// Normal tangent
						m_PositionTangents[KeyIndex] = 0.5f * (PositionKeys[KeyIndex+1] - PositionKeys[KeyIndex-1]);
					}
				}
			}
			else
			{	// Empty tangents
				m_PositionTangents[0] = Vector3.zero;
			}

			// Build rotation tangents
			m_RotationTangents = new Quaternion[RotationKeys.Length][];
			if ( RotationKeys.Length > 1 )
			{
				for ( int KeyIndex=0; KeyIndex < RotationKeys.Length; KeyIndex++ )
				{
					Quaternion[]	Tangents = new Quaternion[4];
					m_RotationTangents[KeyIndex] = Tangents;

					Quaternion	q0 = Quaternion.LookRotation( RotationKeys[Mathf.Max( 0, KeyIndex-1 )] );
					Quaternion	q1 = Quaternion.LookRotation( RotationKeys[KeyIndex] );
					Quaternion	q2 = Quaternion.LookRotation( RotationKeys[Mathf.Min( RotationKeys.Length-1, KeyIndex+1 )] );
					Quaternion	q3 = Quaternion.LookRotation( RotationKeys[Mathf.Min( RotationKeys.Length-1, KeyIndex+2 )] );

					q0 = CheckReverse( q0, q1 ) ? QuatNeg( q0 ) : q0;
					q2 = CheckReverse( q1, q2 ) ? QuatNeg( q2 ) : q2;
					q3 = CheckReverse( q2, q3 ) ? QuatNeg( q3 ) : q3;

					Tangents[0] = q1;
					Quaternion	ConjQ1 = QuatConjuguate( q1 );
					Quaternion	ConjQ2 = QuatConjuguate( q2 );
					Tangents[1] = q1 * QuatExp( QuatScale( -0.25f, QuatAdd( QuatLog( ConjQ1 * q0 ), QuatLog( ConjQ1 * q2 ) ) ) );
					Tangents[2] = q2 * QuatExp( QuatScale( -0.25f, QuatAdd( QuatLog( ConjQ2 * q1 ), QuatLog( ConjQ2 * q3 ) ) ) );
					Tangents[3] = q2;
				}
			}
			else
			{	// Empty tangents
				Quaternion	q = Quaternion.LookRotation( RotationKeys[0] );
				m_RotationTangents[0] = new Quaternion[4] { q, Quaternion.identity, Quaternion.identity, q };
			}
		...
		public Vector3	CSplinePosition( ref Vector3 p0, ref Vector3 p1, ref Vector3 t0, ref Vector3 t1, float t )
		{
			float	r = 1.0f - t;
			float	h00 = (1.0f + 2.0f * t) * r*r;
			float	h10 = t * r*r;
		...
		public Vector3	LerpView( ref Vector3 v0, ref Vector3 v1, float t )
		{
			Vector3	Ortho = Vector3.Cross( v0, v1 ).normalized;
			Vector3	Ortho2 = Vector3.Cross( Ortho, v0 ).normalized;
			float	Alpha = Mathf.Atan2( Vector3.Dot( v1, Ortho2 ), Vector3.Dot( v1, v0 ) ) * t;
			return	Mathf.Cos( Alpha ) * v0.normalized + Mathf.Sin( Alpha ) * Ortho2;
		}

		Quaternion	QuatLog( Quaternion q )
		{
			float	fAngle = Mathf.Acos( q.w );
			float	fSine = Mathf.Sin( fAngle );
			float	fScale =  Mathf.Abs( fSine ) >= float.Epsilon ? fAngle / fSine : 1.0f;
			q.x *= fScale;
			q.y *= fScale;
			q.z *= fScale;
			q.w = 0.0f;
		...
		{
			float	fTheta = Mathf.Sqrt( q.x*q.x + q.y*q.y + q.z*q.z );
			float	fScale = fTheta > float.Epsilon ? Mathf.Sin( fTheta ) / fTheta : 1.0f;
			q.x *= fScale;
			q.y *= fScale;
			q.z *= fScale;
			q.w = Mathf.Cos( fTheta );
			return q;
		}

		Quaternion	QuatConjuguate( Quaternion q )
		{
			return new Quaternion( -q.x, -q.y, -q.z, q.w );
		}

		Quaternion	QuatAdd( Quaternion _Op0, Quaternion _Op1 )
		{ 
			return new Quaternion( _Op0.x + _Op1.x, _Op0.y + _Op1.y, _Op0.z + _Op1.z, _Op0.w + _Op1.w );
		}

		Quaternion	QuatSub( Quaternion _Op0, Quaternion _Op1 )
		{ 
			return new Quaternion( _Op0.x - _Op1.x, _Op0.y - _Op1.y, _Op0.z - _Op1.z, _Op0.w - _Op1.w );
		}

		Quaternion	QuatNeg( Quaternion q )
		{ 
			return new Quaternion( -q.x, -q.y, -q.z, -q.w );
		}

		Quaternion	QuatScale( float s, Quaternion q )
		{ 
		...
		Quaternion	QuatSlerp( Quaternion q0, Quaternion q1, float _t )
		{
			float	fCosine = Quaternion.Dot( q0, q1 );
			float	fSine = Mathf.Sqrt( Mathf.Abs( 1.0f - fCosine*fCosine ) );
			if ( System.Math.Abs( fSine ) < float.Epsilon )
				return	q0;

			float	fAngle = Mathf.Atan2( fSine, fCosine );
			float	fInvSine = 1.0f / fSine;

			float	c0 = Mathf.Sin( (1-_t) * fAngle) * fInvSine;
			float	c1 = Mathf.Sin(   _t   * fAngle) * fInvSine;

			return QuatAdd( QuatScale( c0, q0 ), QuatScale( c1, q1 ) );
		}

		bool		CheckReverse( Quaternion q0, Quaternion q1 )
		{
			float	L0 = QuatLength( QuatAdd( q0, q1 ) );
			float	L1 = QuatLength( QuatSub( q0, q1 ) );
			return L0 < L1;
		}

		public Vector3	QuatSquad( ref Quaternion q0, ref Quaternion q1, ref Quaternion t0, ref Quaternion t1, float t )
		{
			Quaternion	q = QuatSlerp( QuatSlerp( q0, q1, t ), QuatSlerp( t0, t1, t ), 2.0f * t * (1.0f - t) );	// Squad, as explained in http://www.itu.dk/people/erikdam/DOWNLOAD/98-5.pdf pp. 51 (originaly from Shoemake, 1987)

			float	Angle;
			Vector3	Axis;
			q.ToAngleAxis( out Angle, out Axis );
			Angle *= Mathf.Deg2Rad;	// WTH Unity ???!!!

			Vector3	InitialDirection = Vector3.forward;
			Vector3	Ortho = Vector3.Cross( Axis, InitialDirection ).normalized;
			Vector3	FinalDirection = Mathf.Cos( Angle ) * InitialDirection + Mathf.Sin( Angle ) * Ortho;
	...
	protected static DemoPath	PATH0 = new DemoPath() {
		...
		PresetBalanceStart = 0.8f,		PresetBalanceEnd = 0.81f,
		// Time
	...
	//////////////////////////////////////////////////////////////////////////
	// Fixed altitude and Sun, noise tiling changing
	protected static DemoPath	PATH1 = new DemoPath() {
		// Path
		Duration = 10.0f,
		PositionKeys = new Vector3[] { new Vector3( 1882.03f, 129.1f, 1011.17f ), new Vector3( 1882.03f, 129.1f, 1011.17f ) },
		RotationKeys = new Vector3[] { new Vector3( -0.98565f, 0.15273f, 0.07186f ), new Vector3( -0.98565f, 0.15273f, 0.07186f ) },
		// Weather
		PresetSource = NuajOrchestrator.WEATHER_PRESETS.GLAMOUR,
		PresetTarget = NuajOrchestrator.WEATHER_PRESETS.GLAMOUR,
		PresetBalanceStart = 1.0f,		PresetBalanceEnd = 1.0f,
		// Time
		TimeOfDayStart = 7.0f,	TimeOfDayEnd = 16.0f,	NightDensityFactor = 1.0f,
		// Wind
		Wind = 0.0f,	WindAngle = 0.0f,
		// 3D Noise tiling factor
		NoiseTilingFactorStart = 0.5f, NoiseTilingFactorEnd = 2.5f, 
	};
	...
	protected static DemoPath	PATH2 = new DemoPath() {
		...
		PresetBalanceStart = 1.0f,		PresetBalanceEnd = 1.0f,
		// Time
		TimeOfDayStart = 12.0f,	TimeOfDayEnd = -2.0f,	NightDensityFactor = 2.0f,// 2.4f,
		// Wind
	...
	protected static DemoPath	PATH3 = new DemoPath() {
		...
		// Weather
		PresetSource = NuajOrchestrator.WEATHER_PRESETS.SUMMER_CLOUDS,
		PresetTarget = NuajOrchestrator.WEATHER_PRESETS.STORMY,
		PresetBalanceStart = 0.0f,		PresetBalanceEnd = 1.0f,
		// Time
		TimeOfDayStart = 7.5f,	TimeOfDayEnd = 8.0f,	NightDensityFactor = 2.4f,
		// Wind
		Wind = 0.04f,	WindAngle = 0.0f,
	};
	protected static DemoPath	PATH4 = new DemoPath() {
		...
		// Time
		TimeOfDayStart = 8.0f,	TimeOfDayEnd = 9.0f,	NightDensityFactor = 2.4f,
		// Wind
		Wind = 0.04f,	WindAngle = 0.0f,
	};
	...
	protected static DemoPath	PATH7 = new DemoPath() {
		...
		PresetTarget = NuajOrchestrator.WEATHER_PRESETS.CLOUDY,
		PresetBalanceStart = 0.0f,	PresetBalanceEnd = 0.5f,
		// Time
		TimeOfDayStart = 12.0f,	TimeOfDayEnd = 18.0f,	NightDensityFactor = 4.0f,
		// Wind
		Wind = 0.3f,	WindAngle = 0.0f,
	};

	protected static DemoPath	PATH8 = new DemoPath() {
		// Path
		Duration = 10.0f,
		PositionKeys = new Vector3[] { new Vector3( 957.21f, 195.0f, 660.0f ), new Vector3( 957.21f, 195.0f, 660.0f ) },
		RotationKeys = new Vector3[] { new Vector3( 0.4723114f, 0.2034232f, 0.8576368f ), new Vector3( 0.4723114f, 0.2034232f, 0.8576368f ) },
		...
		PresetSource = NuajOrchestrator.WEATHER_PRESETS.VALLEY_MIST,
		PresetTarget = NuajOrchestrator.WEATHER_PRESETS.CLOUDY,
		PresetBalanceStart = 0.5f,	PresetBalanceEnd = 0.6f,
		// Time
		TimeOfDayStart = 18.0f,	TimeOfDayEnd = 19.5f,	NightDensityFactor = 4.0f,
		// Wind
		Wind = 0.3f,	WindAngle = 0.0f,
	};
	...
	{
		protected const float	STRIKE_INTENSITY = 400.0f;			// BLAAAÂÂÂÂST !!!
		...
		public void		Update( CameraFly _Caller, float _DeltaTime, bool _bEnable, float _Importance, float _CloudAltitudeKm, float _CloudThicknessKm )
		{
			...
			{	// Now striking...
				...
				float		BoltAltitude = (_CloudAltitudeKm + 0.5f * _CloudThicknessKm) / _Caller.Manager.WorldUnit2Kilometer;
							BoltPosition.x = StrikePosition.x;
							BoltPosition.y = BoltAltitude;
							BoltPosition.z = StrikePosition.z;
				m_Bolt.transform.position = BoltPosition;

				// Start in the clouds
				m_Bolt.P0 = new Vector3( 0.0f, 0.0f, 0.0f );

				// End in the ground
				m_Bolt.P1 = new Vector3( 0.0f, -BoltAltitude, 0.0f );
