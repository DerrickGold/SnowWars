...
namespace Nuaj.GUI
{
	...
	public class ModuleEditorPerspective : ModuleEditorBase
	{
		...
		protected const float	MAX_RAYLEIGH = 100.0f;
		protected const float	MAX_MIE = 500.0f;
		...
		public override bool	OnInspectorGUIAdvanced()
		{
			...
			if ( m_Module.SkyModel != ModulePerspective.SKY_MODEL.NONE )
			{
				...
				{
					m_Module.ScatteringBoost = GUIHelpers.Slider( new GUIContent( "Scattering Boost", "Gives a boost to the sky scattering" ), m_Module.ScatteringBoost, 0.0f, 2.0f, "Change Scattering Boost" );
					m_Module.ExtinctionBoost = GUIHelpers.Slider( new GUIContent( "Extinction Boost", "Gives a boost to the sky extinction, avoiding dark colors sometimes seen at the horizon" ), m_Module.ExtinctionBoost, 0.0f, 2.0f, "Change Extinction Boost" );
				}

				GUIHelpers.Separate();
				m_Module.UseCustomAmbientSkyColor = GUIHelpers.CheckBox( new GUIContent( "Use Custom Ambient Sky Color", "Tells if we should use the custom ambient sky color instead of the ones that are automatically computed" ), m_Module.UseCustomAmbientSkyColor, (m_Module.UseCustomAmbientSkyColor ? "Disable" : "Enable") + " Custom Ambient Sky Color" );
				m_Module.CustomAmbientSkyColor = GUIHelpers.ColorBox( new GUIContent( "Custom Ambient Sky", "Sets the custom ambient sky color." ), m_Module.CustomAmbientSkyColor, "Change Custom Ambient Sky Color" );
			}
			...
			if ( GUIHelpers.FoldOut( ref m_bFoldOutRendering, "Rendering Parameters" ) )
			{
				...
				GUIHelpers.Separate();
				m_Module.DownsampleFactor = GUIHelpers.SliderWithCheck( "SkyDownsampleFactor", new GUIContent( "Downsample Factor", "Changes the resolution of the rendering" ), m_Module.DownsampleFactor, 0.05f, 1.0f, "Change Downsample Factor" );
				if ( m_Module.SkyModel == ModulePerspective.SKY_MODEL.COMPLEX )
				{
