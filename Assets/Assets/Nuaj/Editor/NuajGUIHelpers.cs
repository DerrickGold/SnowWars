...
namespace Nuaj.GUI
{
	public static class GUIHelpers
	{
		...
		/// This is most important on parameters that control downsample factors so the user understands it's not a small feat to change these values
		/// </summary>
...
//			ShrinkableLabel( _Title );
		...
		{
			return SelectObject<Texture2D>( _Title, _Texture, _UndoName, GUILayout.Width( 75.0f ), GUILayout.Height( 64.0f ) );
		}

		public static Cubemap	SelectCubeMap( GUIContent _Title, Cubemap _Texture, string _UndoName )
		{
			return SelectObject<Cubemap>( _Title, _Texture, _UndoName, GUILayout.Width( 75.0f ), GUILayout.Height( 64.0f ) );
		}
		...
		{
			if ( _UndoName != null )
				Undo.RecordObject( ms_UNDOObject, _UndoName );
		}
	}
}