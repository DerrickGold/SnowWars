	...
	public class ModuleEditorMain : ModuleEditorBase
	{
		...
		protected override void OnInspectorGUICommon()
		{
			...
			if ( GUIHelpers.FoldOut( ref m_bFoldOutSun, "Sun Parameters" ) )
			{
				...
				m_Module.SunsetAngleStart = GUIHelpers.Slider( new GUIContent( "Sunset Start", "Specifies the angle where the Sun starts setting" ), m_Module.SunsetAngleStart, 0.0f, 180.0f, "Change Sunset Start Angle" );
				m_Module.SunsetAngleEnd = GUIHelpers.Slider( new GUIContent( "Sunset End", "Specifies the angle after which we consider the Sun is set" ), m_Module.SunsetAngleEnd, 0.0f, 180.0f, "Change Sunset End Angle" );
			...
			if ( GUIHelpers.FoldOut( ref m_bFoldOutWorld, "World Parameters" ) )
			{
				...
				m_Module.PlanetRadiusOffsetKm = GUIHelpers.Slider( new GUIContent( "Horizon Offset", "Changes the planet horizon offset that allows to trace the sky and clouds even below the default horizon in case you estimate the horizon line is badly placed" ), m_Module.PlanetRadiusOffsetKm, -20.0f, 0.0f, "Change Planet Radius Offset" );
				...
				GUIHelpers.Separate();
				m_Module.UpsamplingTechnique = (NuajManager.UPSAMPLING_TECHNIQUE) GUIHelpers.ComboBox( new GUIContent( "Upsampling Technique", "Changes the technique used to up-sample and refine the downsampled rendering" ), m_Module.UpsamplingTechnique, "Change UpScale Technique" );
				if ( m_Module.UpsamplingTechnique == NuajManager.UPSAMPLING_TECHNIQUE.ACCURATE )
				{
					m_Module.ZBufferDiscrepancyThreshold = GUIHelpers.Slider( new GUIContent( "ZBuffer Threshold", "Changes the threshold that helps to determine which pixels should be recomputed at full resolution\nNOTE: Be careful when dealing with dense foliage or meshes with many holes as each pixel around the holes will be recomputed at full resolution for clean refinement, incuring an increase in rendering time !" ), m_Module.ZBufferDiscrepancyThreshold, 1.0f, 1000.0f, "Change ZBuffer Discrepancy Threshold" );
				...
				else if ( m_Module.UpsamplingTechnique == NuajManager.UPSAMPLING_TECHNIQUE.SMART )
				{
					m_Module.SmartUpsamplingWeightFactor = GUIHelpers.Slider( new GUIContent( "Smart Weight Factor", "Changes the strength of the smart upsampling effect. A weight of 0 should fallback to standard bilinear sampling while a stronger weight will pinpoint the use of one single sample over the others." ), m_Module.SmartUpsamplingWeightFactor, 0.0f, 0.1f, "Change Smart Upsampling Weight Factor" );
					m_Module.SmartUpsamplingCutoutFactor = GUIHelpers.Slider( new GUIContent( "Smart Cutout Factor", "Changes the strength of the sharp cutout for thin objects (reduced flickering but also responsible for darker border)." ), m_Module.SmartUpsamplingCutoutFactor, 0.0f, 1.0f, "Change Smart Upsampling Cutout Factor" );
					m_Module.ShowZBufferDiscrepancies = GUIHelpers.CheckBox( new GUIContent( "Show ZBuffer Discrepancies", "Shows the ZBuffer discrepancies and highlights in red (sky) and green (clouds) the pixels that need to be recomputed at full resolution.\nThis is a helper for you to tweak the threshold nicely depending on the precision of your scene." ), m_Module.ShowZBufferDiscrepancies, "Change Show ZBuffer Discrepancies" );
				}
			...
			if ( GUIHelpers.FoldOut( ref m_bFoldOutShadowMap, "Shadow Map Parameters" ) )
			{
				...
				m_Module.ShadowFarClip = GUIHelpers.Slider( new GUIContent( "Shadow Far Clip", "Sets the the far clip distance for shadows." ), m_Module.ShadowFarClip, 0.0f, 10000.0f, "Change Shadow Far Clip" );
			...
			if ( GUIHelpers.FoldOut( ref m_bFoldOutLuminanceAdaptation, "Luminance Adaptation" ) )
			{
				...
				using ( GUIHelpers.GUIEnabler( m_Module.LuminanceComputationType == NuajManager.LUMINANCE_COMPUTATION_TYPE.DOWNSAMPLE || m_Module.LuminanceComputationType == NuajManager.LUMINANCE_COMPUTATION_TYPE.DOWNSAMPLE_LOG ) )
			...
			if ( GUIHelpers.FoldOut( ref m_bFoldOutToneMapping, "Tone Mapping" ) )
			{
				...
				GUIHelpers.Separate();
				m_Module.ToneMappingGamma = GUIHelpers.Slider( new GUIContent( "Gamma Correction", "Specifies the gamma correction to apply to the luminance." ), m_Module.ToneMappingGamma, 0.0f, 4.0f, "Change Tone Mapping Gamma" );

				GUIHelpers.Separate();
				m_Module.UnitySunColorFactor = GUIHelpers.Slider( new GUIContent( "Sun Color Factor", "Specifies the factor to apply to the tone mapped Sun color to make it a Unity LDR color." ), m_Module.UnitySunColorFactor, 0.0f, 2.0f, "Change Sun Color Factor" );
				m_Module.UnityAmbientColorFactor = GUIHelpers.Slider( new GUIContent( "Ambient Color Factor", "Specifies the factor to apply to the tone mapped Sky color to make it a Unity LDR ambient color." ), m_Module.UnityAmbientColorFactor, 0.0f, 2.0f, "Change Ambient Color Factor" );
