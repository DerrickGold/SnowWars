...
namespace Nuaj.GUI
{
	...
	public class ModuleEditorSatellites : ModuleEditorBase
	{
		...
		{
			using ( GUIHelpers.GUIEnabler( !_Satellite.SimulateLighting || !_Satellite.UseSunLuminance ) )
			...
			GUIHelpers.Separate();
			_Satellite.UseAsNightLightSource = GUIHelpers.CheckBox( new GUIContent( "Use as Night Light", "Tells if the satellite should become the main light source at night." ), _Satellite.UseAsNightLightSource, "Change Use as Night Light" );
			_Satellite.NightLuminanceFactor = GUIHelpers.Slider( new GUIContent( "Night Luminance Factor", "Sets the satellite's luminance factor when used ad the main light source at night." ), _Satellite.NightLuminanceFactor, 0.0f, 0.1f, "Change Satellite Night Luminance Factor" );
		...
		{
			_Satellite.Luminance = GUIHelpers.Slider( new GUIContent( "Luminance", "Sets the satellite's luminance.\nBe careful the luminance should be very inferior to the Sun's intensity (like 100 times lower) otherwise your satellites will start to show during the day." ), _Satellite.Luminance, 0.0f, 0.1f, "Change Satellite Luminance" );
			GUIHelpers.Separate();
