...
namespace Nuaj.GUI
{
	...
	public class ModuleEditorCloudVolume : ModuleEditorBase
	{
		...
		public override bool	OnInspectorGUIAdvanced()
		{
			...
			{
				m_Module.DownsampleFactor = GUIHelpers.SliderWithCheck( "CloudVolumeDownscaleFactor", new GUIContent( "Downsample Factor", "Changes the resolution of the rendering.\nThis has a huge impact on speed but also on cloud blurriness." ), m_Module.DownsampleFactor, 0.05f, 1.0f, "Change Downsample Factor" );
		...
		protected void	DisplayCloudLayer( Nuaj.ModuleCloudVolume.CloudLayer _Layer, int _LayerIndex )
		{
			...
			using ( GUIHelpers.GUIEnabler( _Layer.Enabled ) )
			{
				...
				_Layer.Thickness = GUIHelpers.Slider( new GUIContent( "Thickness", "Sets the cloud's thickness (in kilometers)." ), _Layer.Thickness, 0.0f, 4.0f, "Change Cloud Thickness" );

				if ( GUIHelpers.FoldOut( ref m_bFoldOutShadow[_LayerIndex], "Shadow Map" ) )
				{
					_Layer.ShadowOpacity = GUIHelpers.Slider( new GUIContent( "Shadow Opacity", "Sets the opacity of the shadow map." ), _Layer.ShadowOpacity, 0.0f, 1.0f, "Change Shadow Opacity" );
					...
					_Layer.ShadowStepsCount = GUIHelpers.SliderIntWithCheck( "CloudVolumeShadowStepsCount" + _LayerIndex, new GUIContent( "Shadow Steps Count", "Sets the amount of rendering steps per shadow map texel.\nThis has a strong influence on quality but also on speed." ), _Layer.ShadowStepsCount, 4, 32, "Change Cloud Shadow Steps Count" );
					using ( GUIHelpers.GUIEnabler( _Layer.SmoothShadowMap = GUIHelpers.CheckBox( new GUIContent( "Smooth Shadow Map", "Tells if we should smooth out the shadow map" ), _Layer.SmoothShadowMap, "Change Smooth Shadow Map" ) ) )
						_Layer.ShadowSmoothSize = GUIHelpers.Slider( new GUIContent( "Smooth Size", "Sets the size of the smoothing kernel." ), _Layer.ShadowSmoothSize, 0.0f, 4.0f, "Change Smooth Size" );
				}
				...
				if ( GUIHelpers.FoldOut( ref m_bFoldOutAppearance[_LayerIndex], "Appearance" ) )
				{
					...
					_Layer.CoverageOffset = GUIHelpers.Slider( new GUIContent( "Coverage Offset", "Sets the cloud coverage." ), _Layer.CoverageOffset, -1.0f, 1.0f, "Change Coverage Offset" );
					_Layer.CoverageContrast = GUIHelpers.Slider( new GUIContent( "Coverage Contrast", "Sets the cloud coverage." ), _Layer.CoverageContrast, 0.0f, 4.0f, "Change Coverage Contrast" );
					_Layer.Density = 10.0f * GUIHelpers.Slider( new GUIContent( "Density", "Sets the cloud's density cloud density that influences the capacity of the cloud to absorb and scatter light." ), 0.1f * _Layer.Density, 0.0f, MAX_DENSITY, "Change Cloud Density" );
					_Layer.BevelSoftness = GUIHelpers.Slider( new GUIContent( "Bevel Softness", "Sets the cloud coverage." ), _Layer.BevelSoftness, 0.0f, 1.0f, "Change Bevel Softness" );
				...
				if ( GUIHelpers.FoldOut( ref m_bFoldOutAdvanced[_LayerIndex], "Advanced" ) )
				{
					...
					_Layer.AlbedoDirectional = GUIHelpers.Slider( new GUIContent( "Directional Albedo", "Sets the directional cloud albedo.\nCloud albedo (i.e. the ability of the cloud to scatter and reflect light) is defined as a ratio of the Extinction.\nA value of 0 will yield extremely dark clouds while a value of 1 will reflect all light and absorb nothing (in nature, it is usually considered to be almost 1, because clouds are mostly composed of small water droplets that mainly reflect light : almost no absorption occurs, hence the whiteness of clouds)" ), _Layer.AlbedoDirectional, 0.0f, 1.0f, "Change Directional Cloud Albedo" );
					...
					_Layer.AlbedoIsotropic = GUIHelpers.Slider( new GUIContent( "Isotropic Albedo", "Sets the isotropic cloud albedo.\nCloud albedo (i.e. the ability of the cloud to scatter and reflect light) is defined as a ratio of the Extinction.\nA value of 0 will yield extremely dark clouds while a value of 1 will reflect all light and absorb nothing (in nature, it is usually considered to be almost 1, because clouds are mostly composed of small water droplets that mainly reflect light : almost no absorption occurs, hence the whiteness of clouds)" ), _Layer.AlbedoIsotropic, 0.0f, 1.0f, "Change Isotropic Cloud Albedo" );
					...
					GUIHelpers.Separate();
					GUIHelpers.BoldLabel( new GUIContent( "Isotropic Light Contributions", "" ) );
					_Layer.IsotropicContributionSky = 200.0f * GUIHelpers.Slider( new GUIContent( "Sky Contribution", "Sets the contribution of sky light to isotropic diffusion of light within the clouds." ), 0.005f * _Layer.IsotropicContributionSky, 0.0f, 1.0f, "Change Isotropic Sky Contribution" );
					_Layer.IsotropicContributionSun = 200.0f * GUIHelpers.Slider( new GUIContent( "Sun Contribution", "Sets the contribution of Sun light to isotropic diffusion of light within the clouds." ), 0.005f * _Layer.IsotropicContributionSun, 0.0f, 1.0f, "Change Isotropic Sun Contribution" );
					_Layer.IsotropicContributionTerrain = 2.0f * GUIHelpers.Slider( new GUIContent( "Terrain Contribution", "Sets the contribution of reflected terrain light to isotropic diffusion of light within the clouds." ), 0.5f * _Layer.IsotropicContributionTerrain, 0.0f, 1.0f, "Change Isotropic Terrain Contribution" );
