...
namespace Nuaj
{
	...
	public class ModuleSatellites : ModuleBase, IComparer<ModuleSatellites.SatelliteBase>
	{
		...
		public abstract class	SatelliteBase : IMonoBehaviour
		{
			...
			protected static readonly Vector3	DEFAULT_ROTATION_AXIS = new Vector3( 0.2873479f, 0.9578263f, 0 ).normalized;	// Slightly slanted
			protected const float				DEFAULT_REVOLUTION_ANGLE = -40.0f * Mathf.Deg2Rad;
			...
			protected void	SetCameraTransforms( NuajMaterial _Material )
			{
				_Material.SetMatrix( "_Camera2World", m_Owner.Owner.Camera2World );
				_Material.SetMatrix( "_World2Camera", m_Owner.Owner.World2Camera );
			}
		...
		/// It can be used as an alternate light source at night
		...
		public class	SatellitePlanetaryBody : SatelliteBase
		{
			...
			protected const bool	DEFAULT_USE_AS_NIGHT_LIGHT_SOURCE = true;
			protected const float	DEFAULT_LUMINANCE_FACTOR = 0.01f;
			...
			// Night light source
			[SerializeField] protected bool				m_bUseAsNightLightSource = DEFAULT_USE_AS_NIGHT_LIGHT_SOURCE;
			[SerializeField] protected float			m_NightLuminanceFactor = DEFAULT_LUMINANCE_FACTOR;
			...
			/// <summary>
			/// Tells if this satellite should be used as the main light source at night
			/// </summary>
			public bool						UseAsNightLightSource
			{
				get { return m_bUseAsNightLightSource; }
				set { m_bUseAsNightLightSource = value; }
			}

			/// <summary>
			/// Gets or sets the factor to apply to the Sun's luminance to create the moon luminance for night lighting
			/// </summary>
			public float					NightLuminanceFactor
			{
				get { return m_NightLuminanceFactor; }
				set { m_NightLuminanceFactor = value; }
			}
			...
			public float					SurfaceRoughness
			{
				...
				set
				{
					...
					float	Roughness = 0.5f * Mathf.PI * m_SurfaceRoughness;
			...
			public override void Render()
			{
				...
				float	Luminance = m_bSimulateLighting && m_bUseSunLuminance ? m_Owner.m_Owner.SunIntensity : m_Luminance;

				NuajMaterial	M = m_Owner.m_Material;
				SetCameraTransforms( M );
				...
				GL.Begin( GL.QUADS );
				GL.TexCoord2( -1.0f, -1.0f );	GL.Vertex3( 0.0f, 0.0f, 0.0f );
				GL.TexCoord2( -1.0f, +1.0f );	GL.Vertex3( 0.0f, 0.0f, 0.0f );
				GL.TexCoord2( +1.0f, +1.0f );	GL.Vertex3( 0.0f, 0.0f, 0.0f );
				GL.TexCoord2( +1.0f, -1.0f );	GL.Vertex3( 0.0f, 0.0f, 0.0f );
				GL.End();

				// Setup this satellite as the main light source at night
				if ( !m_bUseAsNightLightSource )
					return;

				float	MoonPhase = Mathf.Clamp01( Mathf.Lerp( 0.2f, 0.8f, m_SurfaceRoughness ) + Vector3.Dot( m_Direction, -m_Owner.m_Owner.SunDirection ) );
				float	LightLuminance = m_NightLuminanceFactor * MoonPhase * Luminance;

				m_Owner.m_Owner.UseMoonAtNight = true;
				m_Owner.m_Owner.MoonDirection = m_Direction;
				m_Owner.m_Owner.MoonColor = LightLuminance * Help.ColorToVec3( m_Albedo );
				m_Owner.m_Owner.MoonIntensity = LightLuminance;
			}
			...
			public override void Reset()
			{
				...
				UseAsNightLightSource = DEFAULT_USE_AS_NIGHT_LIGHT_SOURCE;
				NightLuminanceFactor = DEFAULT_LUMINANCE_FACTOR;
		...
		public class	SatelliteStar : SatelliteBase
		{
			...
			protected override void UpdateCachedValues()
			{
				...
				m_RotationAxisZ = m_Owner.m_Owner.SunDirection;
 				...
 				// Recompute tangent space from revolution axis
				// Disabled as it deforms the sprites
// 				if ( Math.Abs( Vector3.Dot( Vector3.up, m_RotationAxisZ ) ) > 1e-6f )
// 				{
// 					m_RotationAxisX = Vector3.Cross( Vector3.up, m_RotationAxisZ ).normalized;
// 					m_RotationAxisY = Vector3.Cross( m_RotationAxisZ, m_RotationAxisX );
// 				}
// 				else
// 				{	// Singular case at zenith
// 					m_RotationAxisX = Vector3.right;
// 					m_RotationAxisY = Vector3.Cross( m_RotationAxisZ, m_RotationAxisX ).normalized;
// 				}
		...
		public class	SatelliteBackground : SatelliteBase
		{
			...
			protected const float	DEFAULT_GAMMA = 2.7f;
			protected static readonly Color	DEFAULT_AMBIENT = new Color( 0.04f, 0.04f, 0.04f );
			...
			protected override void UpdateCachedValues()
			{
				...
				m_AmbientLuminance = 0.001f * m_Luminance * Help.ColorToVec3( m_Ambient );
			}
