...
//#define SHADER_RELEASE_USE_SEMI_COMPILED	// Define this to use semi-compiled shaders (GLSL still needs full compilation before release)
//#define LOG_DEBUG
...
namespace Nuaj
{
	...
	public static class	Help
	{
...
#if SHADER_RELEASE_USE_SEMI_COMPILED
			_ShaderResourceName = "Shaders_Compiled/" + ShaderName;
#else
		...
		internal static void	DrawIcon( Vector3 _Position, string _ResourceName )
		{
			...
			float		Scale = T.height * ToPoint.magnitude * Mathf.Tan( 0.5f * Mathf.Deg2Rad * Camera.current.fieldOfView ) / Screen.height;
