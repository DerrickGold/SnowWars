...
{
	/// <summary>
	/// This is a wrapper for Unity materials that keeps a cache of NuajTextures to verify if they actually need uploading to the graphics card.
	/// A Nuaj texture gets uploaded only if it's not the same texture as the one from last call OR if the Nuaj texture has its Dirty flag set.
	/// </summary>
	public class	NuajMaterial : IDisposable
	{
		...
		protected static bool		ms_bIsOpenGLChecked = false;
		protected static bool		ms_bIsOpenGL = false;
		...
		/// <summary>
		/// Returns true if the engine is using OpenGL
		/// </summary>
		public static bool		IsOpenGL
		{
			get
			{
				if ( !ms_bIsOpenGLChecked )
				{	// Check and cache...
					ms_bIsOpenGL = SystemInfo.graphicsDeviceVersion.ToLower().Contains( "gl" );
					ms_bIsOpenGLChecked = true;
				}
		...
		/// <summary>
		/// Renders from source to target using the specified material pass rendered into a specific viewport
		/// </summary>
		/// <param name="_Target"></param>
		/// <param name="_PassIndex"></param>
		/// <param name="_Viewport"></param>
		public void		BlitViewport( RenderTexture _Target, int _PassIndex, Rect _Viewport )
		{
			RenderTexture.active = _Target;
			this.SetPass( _PassIndex );

			float	Umin = _Viewport.xMin / _Target.width;
			float	Umax = _Viewport.xMax / _Target.width;
			float	Vmin = _Viewport.yMin / _Target.height;
			float	Vmax = _Viewport.yMax / _Target.height;

			GL.Viewport( _Viewport );
			GL.Begin( GL.QUADS );
			if ( IsOpenGL )
			{	// Flip vertically in OpenGL
				GL.TexCoord2( Umin, Vmin );	GL.Vertex3( -1.0f, -1.0f, 0.0f );
				GL.TexCoord2( Umin, Vmax );	GL.Vertex3( -1.0f, +1.0f, 0.0f );
				GL.TexCoord2( Umax, Vmax );	GL.Vertex3( +1.0f, +1.0f, 0.0f );
				GL.TexCoord2( Umax, Vmin );	GL.Vertex3( +1.0f, -1.0f, 0.0f );
			}
			else
			{
				GL.TexCoord2( Umin, Vmin );	GL.Vertex3( -1.0f, +1.0f, 0.0f );
