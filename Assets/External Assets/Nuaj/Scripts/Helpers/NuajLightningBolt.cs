...
/// Lightning bolts are defined by a segment that emits light along the path (although, at the time, only the START position is used as a light source)
/// </summary>
[ExecuteInEditMode]
public class	NuajLightningBolt : MonoBehaviour
{
	...
	public void		UpdateStrike()
	{
		...
		// Indeed, the rapid variation in intensity is driven by a 2D noise texture in which we're travelling during the strike.
		// The length of the StrikeVariation vector gives the speed of the variation, its original random orientation ensures no two lightning strikes are the same...
		//
		...
		// Compute intensity enveloppe that modulates the intensity variation : the enveloppe is shaped by a rapidly decreasing exponential
	...
	protected void	UpdateCachedValues()
	{
		...
		{	// Also update our child point light if it exists...
			m_PointLight.transform.localPosition = m_Start;	// Set the light at our start position... This should be inside the clouds for maximum effect.
