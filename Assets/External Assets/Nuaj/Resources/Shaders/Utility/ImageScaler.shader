	...
	SubShader
	{
		...
		Pass
		{
			...
			float4	PS( PS_IN _In ) : COLOR
			{
				...
				return SceneLuminanceHDR + lerp( LumAvg, LumMax, _LerpAvgMax );
			}
			ENDCG
		}
		...
		Pass
		{
			...
			float4	PS( PS_IN _In ) : COLOR
			{
				...
				// Return luminance
				return dot( SceneColor, LUMINANCE );
			}
			ENDCG
		}
		...
		// This pass is the last one and downsamples the main texture down to a 1x1 target
